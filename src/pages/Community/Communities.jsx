...
const GLOBAL_COMMUNITIES_PAGE_SIZE = 20;
...
function CommunitiesPage() {
  ...
  const communitiesQuery = useInfiniteQuery({
    queryKey: ["globalCommunities"],
    initialPageParam: null,
    queryFn: async ({ pageParam }) => {
      const res = await communityService.getGlobalCommunities({
        limit: GLOBAL_COMMUNITIES_PAGE_SIZE,
        nextToken: pageParam,
      });

      return {
        ...res,
        communities: await withCommunityOwnerImages(res?.communities || []),
      };
    },
  ...
  useEffect(() => {
    ...
    if (!target || !communitiesQuery.hasNextPage) return;
    ...
    <div className="min-h-screen bg-bg text-t-primary px-6 py-12">
      <div className="max-w-6xl mx-auto space-y-10">
        ...
        <section className="space-y-5">
          {communitiesQuery.status === "pending" && communities.length === 0 ? (
          ...
          {!communitiesQuery.hasNextPage && communities.length > 0 ? (
